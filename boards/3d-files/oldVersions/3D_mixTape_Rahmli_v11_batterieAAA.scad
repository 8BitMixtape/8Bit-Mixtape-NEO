...
plugHeight = 10.5; // [6:10]
tapeHeight = 11.5; // 9.5 for lipo, 11.5 for AAA holder
topThickness = 1.6; 
wallThickness = 2.4; // [1.2:10]
...
module tapeBody(){
  color("Deeppink") 
   translate([40,0,topThickness]) cube([20,wallThickness,tapeHeight+0],false); 
  difference(){ 
    color("Deeppink") 
    translate ([0,0,topThickness]) linear_extrude(height = tapeHeight, center = false, convexity = 10, twist = 0) import("mixtape_case_3D_bottom.dxf", convexity=3);  
    
    translate([11.5,50,0.7*tapeHeight+topThickness+3]) cube([8,42,0.7*tapeHeight],true);
    translate([87.5,50,0.7*tapeHeight+topThickness+3]) cube([8,42,0.7*tapeHeight],true);
     /* 
     if (!usb_charger) {
        translate ([0,0,0.6]) linear_extrude(height = 15, center = false, convexity = 10, twist = 0) import("mixtape_case_3D_MainCase.dxf", convexity=3);
     } else {
         translate ([0,0,0.6]) linear_extrude(height = 15, center = false, convexity = 10, twist = 0) import("mixtape_case_3D_MainCase_USB.dxf", convexity=3);
     }
    */
    //Face hole
    translate([25,65,0.5*tapeHeight+topThickness]) cube([12,1.5,0.7*tapeHeight],true);
    if (!usb_charger) { translate([49.5,65,0.5*tapeHeight+topThickness]) cube([16,1.5,0.7*tapeHeight],true); }
    translate([74.3,65,0.5*tapeHeight+topThickness]) cube([12,1.5,0.7*tapeHeight],true);
      
    translate([36,65,0.5*tapeHeight+topThickness]) cube([4,1.2,0.6*tapeHeight],true);
    translate([63,65,0.5*tapeHeight+topThickness]) cube([4,1.2,0.6*tapeHeight],true);
    
    if (usb_charger) {
        // cutout usb port
        translate([49.5,64,0.5*tapeHeight+topThickness+tapeHeight-3]) cube([20,10,0.7*tapeHeight+4],true);
    }
    if (cutout_aux_ports) {
        translate([1.5,21.5,0.7*tapeHeight+topThickness+3]) cube([20,12,0.5*tapeHeight],true);
        translate([90,21.5,0.7*tapeHeight+topThickness+3]) cube([20,12,0.5*tapeHeight],true);      
    }
    
    // Battery space
    translate([49.5,23.5,(tapeHeight+topThickness)/2]) cube([98-2*wallThickness,47-2*wallThickness,tapeHeight+topThickness+0.01],true);
    
    translate([49.5,50-wallThickness,(tapeHeight+topThickness)/2])  cube([68-2*wallThickness,30,tapeHeight+topThickness+0.01],true);
  ...
  //color("DarkKhaki") 
  if (usb_charger) {
    //translate([40,0.05,1.6]) cube([20,1.6,tapeHeight-0],false);
  }
}
...
difference(){ 
 color("Deeppink") 
     translate ([0,0,-1.5]) linear_extrude(height = topThickness, center = false, convexity = 10, twist = 0) import("mixtape_case_3D_TopTrapez.dxf", convexity=3);
...
difference(){    
  color("Deeppink") 
     translate ([0,0,0]) linear_extrude(height = topThickness, center = false, convexity = 10, twist = 0) import("mixtape_case_3D_bottom.dxf", convexity=3);
  ...
  translate ([0,0,-3]) linear_extrude(height = 15.5, center = false, convexity = 10, twist = 0) import("mixtape_case_3D_Wheels.dxf", convexity=3); 
}

  translate ([0,0,0.4]) linear_extrude(height = topThickness-0.4, center = false, convexity = 10, twist = 0) import("mixtape_case_3D_NutWheels.dxf", convexity=3); 
